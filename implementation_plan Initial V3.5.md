...
## [ Phase 13: Intégration API ] (Prochaine Étape)

L'interface a été entièrement repensée (Mobile-First, Cartes, Skeletons, Fast-Tap). La prochaine étape est de remplacer les données factices (mocks) par les vraies données provenant du backend Django.

### Objectifs

1.  **Génération des Modèles Typescript** : Créer les interfaces pour `Chant`, `SeanceChant`, `Repetition`, `Presence`, `Mouvement`, `CampagneCotisation`, `Cotisation`.
2.  **Création des Services Angular** :
    *   `MusiqueService` : `getChants()`, `getChantById(id)`, `ajouterChant(data)`, etc.
    *   `PresencesService` : `getRepetitions()`, `getPointage(repetitionId)`, `enregistrerPointage(repetitionId, data)`.
    *   `FinancesService` : `getDashboardKPIs()`, `getDernieresOperations()`, `getCampagnesActives()`.
3.  **Connexion aux Composants** :
    *   `RepertoireListComponent` : Afficher les chants de la DB, configurer la recherche/filtre.
    *   `PresencesListComponent` : Lier la grille Fast-Tap à la répétition en cours (ou la dernière). Permettre d'enregistrer le résultat global.
    *   `FinancesDashboardComponent` : Récupérer les KPI via un endpoint backend aggrégé (ou aggrégation côté front via RxJS `forkJoin`).

### User Review Required
> [!IMPORTANT]
> Es-tu d'accord pour que l'on passe à l'intégration réelle avec le backend ? Une fois validé, nous créerons les services Angular et remplacerons les tableaux statiques (`membresDemo`, `chantsDemo` etc.) par des signaux alimentés via HTTP.
